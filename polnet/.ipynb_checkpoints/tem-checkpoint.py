"""
Models for Transmission Electron Microscopy
...
## IMOD commands
IMOD_CMD_XYZPROJ = '/usr/local/IMOD/bin/xyzproj'
IMOD_CMD_TILT = '/usr/local/IMOD/bin/tilt'
IMOD_CMD_AHEADER = '/usr/local/IMOD/bin/alterheader'
    ...
    def gen_tilt_series_imod(self, vol, angs, ax='X', mode='real'):
        ...
        try:
            with open(self.__log_file, 'a') as file_log:
                ...
                file_log.write('\nInput volume path: ' + in_vol_path)
                file_log.write('\nOutput micrographs path: ' + out_vol_path)
                file_log.write('\nTilt angles: ' + tangles + '\n')
                result = subprocess.run(xyzproj_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                file_log.write(result.stdout.decode())
                file_log.write(result.stderr.decode())
                file_log.flush()  # Ensure logs are written
            self.__save_tangs_file(angs)
        except subprocess.CalledProcessError as e:
            print(f'ERROR: Error calling the command: {xyzproj_cmd}. Error: {e}')
            raise
        except FileNotFoundError as e:
            print(f'ERROR: File not found: {e}')
            raise
        except IOError as e:
            print(f'ERROR: Log file could not be written: {self.__log_file}. Error: {e}')
            raise
