...
def gaussian_distribution(mean, std_dev, shape):
    return cp.random.normal(loc=mean, scale=std_dev, size=shape)

def co_occurrence_distribution(A_coords, B_density, radius):
    kdtree = cKDTree(A_coords)
    B_coords = []
    for A_point in A_coords:
        if cp.random.random() < B_density:
            B_coords.append(A_point + cp.random.uniform(-radius, radius, A_point.shape))
    return cp.array(B_coords)



def add_noise_to_model(vertices, noise_level=0.01):
    noise = cp.random.normal(scale=noise_level, size=vertices.shape)
    return vertices + noise

def generate_random_shape(shape=(64, 64, 64), scale=10):
    noise = cp.random.normal(size=shape)
    verts, faces, normals, values = measure.marching_cubes(noise.get(), level=0)
    return verts, faces

def combine_with_random_shape(base_model, random_shape, weight=0.5):
    combined = (1 - weight) * base_model + weight * random_shape
    return combined
