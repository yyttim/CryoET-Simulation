...
import cupy as cp
from .special_distributions import OccGen  # 导入特殊分布模块
...
class Polymer(ABC):
    ...
    def distribute_monomers(self, base_model, noise_level=0.01, random_shape_weight=0.5):
            # 生成随机形状
            random_shape_verts, random_shape_faces = generate_random_shape()
    
            # 结合模型和随机形状
            enhanced_model = combine_with_random_shape(base_model, random_shape_verts, random_shape_weight)
    
            # 对增强的模型添加噪声
            noisy_model = add_noise_to_model(enhanced_model, noise_level)
    ...
    def insert_molecules_with_distribution(self, molecule_list, distribution_method, **kwargs):
        occ_gen = OccGen([0, 100])
        return occ_gen.insert_molecules(molecule_list, distribution_method, **kwargs)
...
class FiberUnitSDimer(FiberUnit):
    ...
    def __gen_sdimer(self):
        ...
        self.__surf = iso_surface(self.__tomo, .25)
