...
import os
import random
from polnet.utils import lio, pp

# 假设细胞器模型文件名存储在列表中
ORGANELLE_LIST = ['nucleus.mrc', 'mitochondrion.mrc', 'golgi.mrc']

def generate_nonuniform_positions(num_organelles, mean, std_dev):
    positions = np.random.normal(loc=mean, scale=std_dev, size=(num_organelles, 3))
    positions = np.clip(positions, 0, 1)
    return positions

def add_organelles(tomo_den, organelle_files, num_organelles, mean, std_dev):
    positions = generate_nonuniform_positions(num_organelles, mean, std_dev)
    for organelle_file in organelle_files:
        model_path = os.path.join(ROOT_PATH, organelle_file)
        try:
            model = lio.load_mrc(model_path)
        except FileNotFoundError:
            print(f"Model file {organelle_file} not found at {model_path}")
            continue
        model = lin_map(model, lb=0, ub=1)
        model = vol_cube(model)
        model_mask = model < 0.5  # 示例阈值，实际根据模型调整
        model[model_mask] = 0
        for pos in positions:
            # 将细胞器插入到指定位置
            insert_position = (pos * np.array(tomo_den.shape)).astype(int)
            x, y, z = insert_position
            dx, dy, dz = model.shape
            tomo_den[x:x+dx, y:y+dy, z+dz] = np.maximum(tomo_den[x:x+dx, y:y+dy, z+dz], model)
    return tomo_den
...
DETECTOR_SNR = [1.0, 2.0]
MALIGN_MN = 1
MALIGN_MX = 1.5
MALIGN_SG = 0.2

# OUTPUT FILES
OUT_DIR = os.path.realpath(ROOT_PATH + '/data_generated/all_v4')
...
for tomod_id in range(NTOMOS):
    ...
    mean = [0.5, 0.5, 0.5]
    std_dev = [0.1, 0.1, 0.1]
    tomo_den = add_organelles(tomo_den, ORGANELLE_LIST, num_organelles, mean, std_dev)
    ...
    count_prots = 0
    model_surfs, models, model_masks, model_codes = list(), list(), list(), list(), list()
    ...
    if mbs_vtp is None:
        ...
        for p_id, p_file in enumerate(MB_PROTEINS_LIST):
            ...
            center = protein.get_mmer_center()
            if center is None:
